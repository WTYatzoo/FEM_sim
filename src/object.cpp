...
object::object(std::string input_dir,double dt,double density,int line_search,double weight_line_search,std::string constitutive_model)
{
  while(!myvertexs.empty())
    {
      myvertexs.pop_back();
...
int object::hex_prepare()
{
  size_t i;
  for(i=0;i<num_hexahedrons;++i)
    {
      myhexahedrons[i].prepare(shapeFuncGrad,myvertexs);
    }
  return 0;
}
int object::prepare()
{
  ...
  if(constitutive_model=="linear"||constitutive_model=="linear_with_stiffness_tensor")
    ...
    {
      max_iteration_num=1; // 40 original  
    }
...
int object::calMassForVertex()
{
  ...
  for(i=0;i<num_hexahedrons;++i)
    {
      double vol=myhexahedrons[i].avg_det_pX_peps*8;
      for(a=0;a<2;++a)
	{
	  for(b=0;b<2;++b)
	    {
...
int object::calJacobianAndHessianForHex(Eigen::MatrixXd &Hessian,Eigen::VectorXd &Jacobian)
{
  ...
  for(i=0;i<num_hexahedrons;++i)
    {
      myhexahedrons[i].calJacobianAndHessian(shapeFuncGrad,myvertexs,Hessian,Jacobian,constitutive_model);
    }
...
bool object::checkInversion(Eigen::VectorXd &dx,Eigen::VectorXd &Jacobian)
{
  ...
  for(i=0;i<num_hexahedrons;++i)
    {         
      if(myhexahedrons[i].checkInversion(shapeFuncGrad,myvertexs,Jacobian,constitutive_model))
	{
	  return true;
	}      
    }  
...
bool object::checkInversion(Eigen::VectorXd &dx)
{
  ...
  for(i=0;i<num_hexahedrons;++i)
    {         
      if(myhexahedrons[i].checkInversion(shapeFuncGrad,myvertexs,constitutive_model))
	{
	  return true;
	}      
    }  
...
double object::calEnergyDif()
{
  ...
  printf("[elasticity potential energy]: old energy:%lf new energy:%lf  \n",energy_old,energy_new);
  double EPS=1e-5; // local constant to judge zero for dt-100
  double d1dt;
  if(fabs(dt-100)<EPS) 
    {  
  ...
  printf("[elasticity potential energy + kinetic energy]: old energy:%lf new energy:%lf \n",energy_old,energy_new);
  for(i=0;i<num_vertex;++i)
    {
   ...
   printf("[elasticity potential energy + kinetic energy + external force potential energy]: old energy:%lf new energy:%lf \n\n",energy_old,energy_new);
