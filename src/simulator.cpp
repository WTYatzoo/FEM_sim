...
int simulator::simulate(boost::property_tree::ptree &para_tree)
{
  ...
  int level=para_tree.get<int>("level.value",1);
  ...
  size_t i,j,k;
  double elasticity_potential_energy;

  object* test_obj=new object(input_object,dt,density,line_search,weight_line_search,constitutive_model);
  
  if(input_mat!="")
    {
      printf("Given material parameter from file\n");
      myio.getMatPara(test_obj,input_mat,input_mat_kind);
    }
  else
    {
      printf("Use the default material parameter\n");
	...
	for(i=0;i<test_obj->num_vertex;++i)
	  {
	    ...
	    //  printf("mass: %lf",test_obj->myvertexs[i].mass);
  ...
  for(i=0;i<frame;++i)
    {
      ...
      elasticity_potential_energy=test_obj->calElasticEnergy();
      printf("elasticity potential energy is :%lf \n ",elasticity_potential_energy);
      ...
      if(simulation_type=="dynamic")
	{
	  ...
	  myio.saveAsVTKwithForce(test_obj,path_name);
